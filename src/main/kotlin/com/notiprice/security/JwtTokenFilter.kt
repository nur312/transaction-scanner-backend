...
class JwtTokenFilter : OncePerRequestFilter() {
    ...
    private fun getUserDetails(token: String): UserDetails {
        val userDetails = Operator()
        val jwtSubject = jwtUtil!!.getSubject(token)!!.split(",").toTypedArray()
        userDetails.id = jwtSubject[0].toLong()
        userDetails.operatorUsername = jwtSubject[1]
        return userDetails
    }
}